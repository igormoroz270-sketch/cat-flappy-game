...
canvas.addEventListener("click", e=>{
  ...
  if(gameState==="game" && gameOver) startGame();
});
...
function startGame(){
  ...
  bullets=[]; enemies=[]; coins=[]; powerUps=[]; explosions=[]; boss=null;
  score=0;
}
...
function rectHit(a,b){ return a.x<a.x+b.w && a.x+a.w>b.x && a.y<b.y+b.h && a.y+a.h>b.y; }
...
/* ================= SHOP ================= */
function drawShop(){
  ctx.fillStyle="rgba(0,0,0,0.95)"; ctx.fillRect(20,100,320,440);
  ctx.fillStyle="white"; ctx.font="28px Arial"; ctx.textAlign="center";
  ctx.fillText("МАГАЗИН", WIDTH/2, 140);

  let startY=180; ctx.font="20px Arial"; ctx.textAlign="left"; let index=0;
...
function drawShopButton(x,y,w,h,text,active){
  ...
  ctx.fillRect(x,y,w,h); ctx.strokeStyle="white"; ctx.strokeRect(x,y,w,h);
  ctx.fillStyle=active?"white":"gray"; ctx.font="16px Arial"; ctx.textAlign="center";
  ctx.fillText(text,x+w/2,y+20);
}
function handleShopClick(x,y){
  let startY=180,index=0;
  for(let key in upgrades){
    const upg=upgrades[key]; const btnX=200, btnY=startY+index*60-25, btnW=90, btnH=30;
...
function drawGameOver(){
  ...
  ctx.font="16px Arial"; ctx.fillText("Нажмите, чтобы перезапустить",70,340);
}
...
function updateDifficulty(){ enemyLevel=1+Math.floor(score/20); }

/* ================= SHOOT ================= */
function autoShoot(){
  if(player.fireCooldown-->0) return;
  bullets.push({x:player.x+player.w/2-2, y:player.y, w:4, h:10, speed:10, super:activeBonuses.superShot.active});
...
function updateEnemies(){
  enemies.forEach((e,ei)=>{
    ...
    bullets.forEach((b,bi)=>{
      if(rectHit(b,e)){ b.super?e.hp=0:e.hp--; bullets.splice(bi,1);
        if(e.hp<=0){
          createExplosion(e.x+e.w/2,e.y+e.h/2);
          // падают монетки
          let coinReward = Math.floor(Math.random()*3+3);
          for(let i=0;i<coinReward;i++){
            coins.push({x:e.x+Math.random()*e.w, y:e.y+Math.random()*e.h, w:10,h:10,speed:2});
          }
          score += coinReward;
          enemies.splice(ei,1);
        }
    ...
    if(!activeBonuses.invincible.active && rectHit(player,e)){ enemies.splice(ei,1); player.hp--; player.tookDamage=true;
      if(player.hp<=0){ gameOver=true; savedCoins+=score; localStorage.setItem("coins",savedCoins); }
    }
  });
}

/* ================= BOSS ================= */
function spawnBoss(){ if(!boss && score>0 && score%50===0) boss={x:WIDTH/2-50,y:-100,w:100,h:100,hp:50+enemyLevel*10,maxHp:50+enemyLevel*10,speed:1}; }
function updateBoss(){
  if(!boss) return;
  boss.y+=boss.speed;
  bullets.forEach((b,bi)=>{
    if(rectHit(b,boss)){ b.super?boss.hp=0:boss.hp--; bullets.splice(bi,1);
      if(boss.hp<=0){ createExplosion(boss.x+boss.w/2,boss.y+boss.h/2); score+=10+enemyLevel*2; boss=null; }
    }
  });
}

/* ================= POWERUPS ================= */
function spawnPowerUp(){ if(Math.random()<0.005){ const types=["invincible","superShot","doubleCoins"]; powerUps.push({x:Math.random()*(WIDTH-20),y:-20,type:types[Math.floor(Math.random()*types.length)],speed:2}); } }
function updatePowerUps(){ powerUps.forEach((p,i)=>{ p.y+=p.speed; if(rectHit(player,p)){ if(p.type==="invincible"){ activeBonuses.invincible.active=true; activeBonuses.invincible.timer=240; } if(p.type==="superShot"){ activeBonuses.superShot.active=true; activeBonuses.superShot.timer=60; } if(p.type==="doubleCoins"){ activeBonuses.doubleCoins.active=true; activeBonuses.doubleCoins.timer=360; } powerUps.splice(i,1); } if(p.y>HEIGHT) powerUps.splice(i,1); }); }

/* ================= COINS ================= */
function updateCoins(){ coins.forEach((c,i)=>{ c.y+=c.speed||2; c.x+=Math.sin(Date.now()/200+c.y/5)*1.5; if(rectHit(player,c)){coins.splice(i,1);score++;} if(c.y>HEIGHT) coins.splice(i,1); }); }
...
function drawGame(){
  ...
  // Игрок — рисуем пиксельный корабль
  ctx.fillStyle="#00ffff"; ctx.fillRect(player.x,player.y,player.w,player.h);
  ctx.fillStyle="#0088ff"; ctx.fillRect(player.x+10,player.y+5,30,40); // кокпит
  ...
  coins.forEach(c=>{ ctx.fillStyle="gold"; ctx.fillRect(c.x,c.y,c.w,c.h); });
  ...
  ctx.fillStyle="white"; ctx.font="16px Arial"; ctx.fillText("❤️ ".repeat(player.hp),10,20);
